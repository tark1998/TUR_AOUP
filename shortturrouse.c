...
int main(int argc, char **argv)
{
        ...
        dump1=(int)(0.01/dt);//T=(int)(pow(N,2)*gamma/dt);
        dump3=(int)(0.0001/dt);
        dump4=(int)(0.000001/dt);//T=(int)(pow(N,2)*gamma/dt);
        T1 = (long long)(10/dt);
        T3 = (long long)(0.1/dt);
        T4 = (long long)(0.001/dt);
        ...
        for(j=rank*spr;j<(rank+1)*spr;j++){
                ...
                n=0;
                sprintf(NAME1,"./traj_data/dump100/dump100_N%dtaur%.4fv%.4lfl%ddt%.4fF%.4f_%d.xyz",N,1.0/gammar,v,legs,0.01,F,j);
                printf("%s\n",NAME1);
                ofp=fopen(NAME1,"r");
                if(ofp==NULL){
                        x[0]=0.0;       
                ...
                //sprintf(NAME3,"./dump0.01/taetaxdot_N%dtaur%.4fv%.4lfl%ddt%.4fF%.4f_%d.xyz",N,1/gammar,v,legs,dt,F,j);
                //ofpeta=fopen(NAME3,"w");
                //sprintf(NAME3,"./dump0.01/dump0.01_N%dtaur%.4fv%.4lfl%ddt%.4fF%.4f_%d.xyz",N,1/gammar,v,legs,dt,F,j);
                //ofp1=fopen(NAME3,"w");
                sprintf(NAME3,"./traj_data/dump0.0001/dump0.0001_N%dtaur%.4fv%.4lfl%ddt%.4fF%.4f_%d.xyz",N,1/gammar,v,legs,dt,F,j);
                ofp3=fopen(NAME3,"w");
                sprintf(NAME4,"./traj_data/dump0.000001/dump0.000001_N%dtaur%.4fv%.4lfl%ddt%.4fF%.4f_%d.xyz",N,1/gammar,v,legs,dt,F,j);
                ofp4=fopen(NAME4,"w");
                                
                                
                for(t=1;t<T3+1+plustime;t++){
