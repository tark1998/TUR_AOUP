...
int main(int argc,char **argv){
        ...
        int eof_check;
        int sete,lag;
        double sum, sumx, sumy, sumz, dx2,k,a,gamma,v,q, scale;
        double gammar;
        double dt,F;
        ...
        char OPENF[10000];
        char *file_base[4] = {"0.000001","0.0001","0.01"};
        char file_base_[500];
        if(argc<1){
        ...
        for (p=0; p<2; p++){
                ...
                for(i=0;i<100;i++){
                        ...
                        sprintf(OPENF,"./traj_data/dump%s/dump%s_N%dtaur%.4fv%.4lfl%ddt0.0000F%.4f_%d.xyz",file_base_,file_base_,N,1/gammar,v,legs,F,i);
                        ifp=fopen(OPENF,"r");
                        if (ifp==NULL){
                               continue; 
                        }
                ...
                sprintf(NAME,"./msd_data/msx%s_middle_N%dtaur%.4fv%.4lfl%ddt0.0000F%.4f.dat",file_base[p],(int)(1+legs*ptl_num*scale),1/gammar,v/scale,legs,F);
                data1 = fopen(NAME,"w");
                sprintf(NAME,"./msd_data/cur%s_middle_N%dtaur%.4fv%.4lfl%ddt0.0000F%.4f.dat",file_base[p],(int)(1+legs*ptl_num*scale),1/gammar,v/scale,legs,F);
                data2 = fopen(NAME,"w");
                for(lag=1;lag<size_;lag++){
                        ...
                        fprintf(data2,"%f\t%.12lf\n",lag * atof(file_base[p]) * pow(scale,2),tamdx2[lag-1]/(double)file_cnt); 
                }
                fclose(data1);
                fclose(data2);
                free(tamsd2);
